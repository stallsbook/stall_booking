						...
						<div class="row">
							<div class="col-xl-12 col-lg-12">
								...
								<div class="box box-widget">
									<?php
                                    $viewReview = $cnn -> countrow("SELECT * FROM user_master um , event_master em , review_ratting_master rm where em.event_id = rm.event_id AND um.user_id = rm.user_id AND em.event_id='$event_id'");
                                    if($viewReview > 0)
                                    {
                                        $selectevent = $cnn -> getrows("SELECT * FROM user_master um , event_master em , review_ratting_master rm where em.event_id = rm.event_id AND um.user_id = rm.user_id AND em.event_id='$event_id'");
                                       while($getReview = mysqli_fetch_array($selectevent))
                                       {
                                            
                                    ?>
									<div class="box-header with-border" style="border: 1px solid;">
											...
											<div class="col-md-6">
												...
												<?php
													...
													for($i = 1; $i <= 5; $i++)
													{
													  if($i <= $star)
													  { ?>
												<i class="fa fa-star" aria-hidden="true" style="color:#f9a763;"></i>
												<?php }
													else
													{ ?>
