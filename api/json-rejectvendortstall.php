<?php
	...
	//$updateStatus1 = $cnn -> updatedeleterows("UPDATE event_master SET avai_stall='$minus' WHERE event_id = '$event_id'");
	$updateStatus = $cnn -> updatedeleterows("Delete from  booking_master  WHERE book_id = '$book_id'");
	
	if($updateStatus)
	{
		$gcmRegID=$getBook['deviceid'];
		$Message= $event.' booking rejected "," Reason: '.$reason;
		...
		$message .= '<img src="http://alexinfosoft.com/stall_booking/stall_live/images/logo.png" style="width:100px;height:40px;float:left;" ><h2 style="text-align:center;">Hi, '.$name.' </h2>';
			...
			$message .= '<p> Your Booking Is Rejected In '.$event.' Event.</p>';
